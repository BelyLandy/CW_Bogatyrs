...
public class DollarOneRecognizer : Recognizer, IRecognizer
{
        ...
        float angle)
    {
        ...
        // Приводим шаблоны к обработанному виду для сравнения
        ...
        double score = 1 - (bestDistance / (0.5 * Math.Sqrt(2 * size * size)));

        // Если score ниже порогового значения, считаем фигуру нераспознанной
        float threshold = 0.7f; // пороговое значение (настройте по необходимости)
        if (score < threshold)
        {
            return ("Unrecognized", (float)score);
        }

        return (bestTemplate.Name, (float)score);
    }
